...
class DCNN(object):
    ...
    def train(self,dataset_path,num_classes,batch_size,lr_base,lr_decay,step_size,\
              max_iteration,weight_decay,pretrained_model=None):
        '''
        @description: 构建ResNet50网络结构，训练网络模型，输出训练过程中的logs，保存网络模型
        @params:
        ...
        res_net = ResNet(weight_decay,0.5,num_classes)
...
#                    break
    ...
    def test(self,test_file_path,num_classes,batch_size,model):
        '''
        @description: 构建ResNet50网络结构，加载训练好的网络模型，对测试样本进行分类，输出测试准确率
        @params:
        ...
        res_net = ResNet(weight_decay=0.0,keep_prob=1.0,num_classes=num_classes)
        with tf.Graph().as_default(),tf.device('/gpu:0'):
            ...
            _, prob = res_net.res_net_50(x,is_train=False)
...
if __name__ == '__main__':
    dcnn = DCNN()
    dcnn.train(dataset_path='./data/',num_classes=3037,batch_size=10,lr_base=0.01,\
               lr_decay=0.95,step_size=100000,max_iteration=400000,\
               weight_decay=0.0001,pretrained_model=None)
